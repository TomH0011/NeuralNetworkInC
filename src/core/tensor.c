...
void d_transpose(float *d_input, float *d_output, int rows, int cols);
void addMatrix_GPU(float *data_1, float *data_2, float *output, int rows, int cols);
void scaleTensor_GPU_Wrapper(float *data, float *output, float scalar, int rows, int cols);
...
void deleteTensor(Tensor *tensor) {
    ...
    if (tensor->isOwner == 1) {
        if (tensor->is_gpu) {
            // Data is on GPU -> Use CUDA free
            cudaFree(tensor->data);
        }
        else {free(tensor->data);}
    }
    free(tensor);
}
...
Tensor *matrixAdd_GPU(Tensor *tensorA, Tensor *tensorB) {
    ...
    if (tensorA->is_gpu == 1 || tensorB->is_gpu == 1) {
        printf("Tensor A or Tensor B is not on GPU.\n");
        return NULL;
    }
    if (tensorA->nDim != tensorB->nDim || tensorA->shape != tensorB->shape) {
        printf("invalid shape, or invalid dimensions between tensorA and tensorB");
        return NULL;
    }

    const int rows = tensorA->shape[0];
    const int cols = tensorB->shape[0];

    const size_t newShape[] = {(size_t)rows, (size_t)cols};
    Tensor *output = createTensor(2, newShape);
    free(output->data);

    cudaMalloc((void**)&output->data, sizeof(float) * tensorA->total);

    addMatrix_GPU(tensorA->data, tensorB->data, output->data, rows, cols);
    output->is_gpu = 1;
    output->isOwner = 1;
    return output;
}
...
Tensor *scaleTensor_GPU(Tensor *tensor, const float scalar) {
    ...
    cudaMalloc((void**)&output->data, sizeof(float) * tensor->total);

    scaleTensor_GPU_Wrapper(tensor->data, output->data, scalar, rows, cols);
    output->is_gpu = 1;
    output->isOwner = 1;
...
// multiply two 2D matrices together efficiently
Tensor *matMul2D_GPU(Tensor *A, Tensor *B) {
    if (!A || !B || !A->is_gpu || !B->is_gpu) return NULL;
    if (A->nDim != 2 || B->nDim != 2) {
        printf("GPU MatMul currently supports only 2D tensors.\n");
        return NULL;
    }
    // Shared dimension check: Cols of A == Rows of B
    if (A->shape[1] != B->shape[0]) {
        printf("Shape Mismatch: (%zu, %zu) vs (%zu, %zu)\n",
               A->shape[0], A->shape[1], B->shape[0], B->shape[1]);
        return NULL;
    }

    int M = (int)A->shape[0]; // Output Rows
    int K = (int)A->shape[1]; // Shared Dimension
    int N = (int)B->shape[1]; // Output Cols

    // Output shape is (Rows of A, Cols of B) -> (M, N)
    size_t newShape[] = { (size_t)M, (size_t)N };

    Tensor *C = createTensor(2, newShape);

    // Free the CPU data immediately and alloc GPU
    free(C->data);
    cudaMalloc((void**)&C->data, C->total * sizeof(float));
    C->is_gpu = 1;
    C->isOwner = 1;

    matmul_GPU_wrapper(A->data, B->data, C->data, M, N, K);
...
Tensor *tensorTranspose2D_GPU(const Tensor *A) {
    if (!A || !A->is_gpu || A->nDim != 2) {
        printf("Error: Invalid input for GPU transpose.\n");
        return NULL;
    }

    const int rows = (int)A->shape[0];
    const int cols = (int)A->shape[1];

    // Input: (rows, cols) -> Output: (cols, rows)
    const size_t newShape[] = { (size_t)cols, (size_t)rows };
    Tensor *out = createTensor(2, newShape);
    free(out->data);

    cudaError_t err = cudaMalloc((void**)&out->data, out->total * sizeof(float));
    if (err != cudaSuccess) {
    ...
    d_transpose(A->data, out->data, rows, cols);
...
Tensor *tensorToGPU(const Tensor *cpuTensor) {
    // if tensor already on gpu, dont run this
    if (!cpuTensor || cpuTensor->is_gpu) return NULL;

    Tensor *gpuTensor = createTensor(cpuTensor->nDim, cpuTensor->shape);

    free(gpuTensor->data);

    const cudaError_t err = cudaMalloc((void**)&gpuTensor->data, cpuTensor->total * sizeof(float));
    if (err != cudaSuccess) {
        printf("CUDA Malloc failed: %s\n", cudaGetErrorString(err));
        // clean up struct
        gpuTensor->data = NULL; // prevent double free
        deleteTensor(gpuTensor);
        return NULL;
    }

    // now the actual copying from cpu to gpu
    cudaMemcpy(gpuTensor->data, cpuTensor->data, cpuTensor->total * sizeof(float), cudaMemcpyHostToDevice);
...
Tensor *tensorToCPU(const Tensor *gpuTensor) {
    if (!gpuTensor || !gpuTensor->is_gpu) return NULL;

    // Create struct which allocates CPU RAM by default... what we want
    Tensor *cpuTensor = createTensor(gpuTensor->nDim, gpuTensor->shape);

    // copy data Device -> Host
    cudaMemcpy(cpuTensor->data, gpuTensor->data, gpuTensor->total * sizeof(float), cudaMemcpyDeviceToHost);
